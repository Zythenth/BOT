...
export function registerMessageHandler(client: Client, config: AppConfig): void {
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    const potentialPrefixCommand = isPotentialPrefixCommand(
      content,
      prefix,
    ...
    const parsedCommand =
      parsePrefixCommand(content, prefix) ??
      parseDefaultPrefixFallback(content, prefix, config.defaultPrefix) ??
      parseBotMentionPrefixCommand(content, client.user?.id);
...
function parseDefaultPrefixFallback(content: string, prefix: string, defaultPrefix: string) {
  if (prefix === defaultPrefix) {
    return null;
  }

  return parsePrefixCommand(content, defaultPrefix);
}
...
function warnOnceAboutMissingMessageContent(guildId: string): void {
  ...
  logger.warn("Received guild message without content. Prefix commands need Message Content Intent and channel read access.", {
    guildId
  });
}
...
function isPotentialPrefixCommand(
  ...
  if (defaultPrefix && content.startsWith(defaultPrefix)) {
    return true;
  }

  return Boolean(
    botUserId &&
      (content.startsWith(`<@${botUserId}>`) || content.startsWith(`<@!${botUserId}>`))
  );
}
